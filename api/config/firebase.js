...
if (!admin.apps.length) {
    try {
        ...
        // Cas 0 : Priorité absolue si la variable Render/ENV 'FIREBASE_SERVICE_ACCOUNT' existe
        if (process.env.FIREBASE_SERVICE_ACCOUNT) {
            ...
            try {
                ...
                console.log("Firebase Admin initialisé via la variable FIREBASE_SERVICE_ACCOUNT (Render)");
            } catch (jsonErr) {
                console.error("Erreur de parsing de FIREBASE_SERVICE_ACCOUNT:", jsonErr.message);
            }
        }

        // Si Firebase n'est toujours pas initialisé, passer aux modes suivants
        if (!admin.apps.length) {
            ...
            // Cas B : Mode Vercel / Variables d'environnement séparées
            else {
