...
const Users = () => {
    ...
    const logAdminAction = async (userName, action) => {
        try {
            ...
            console.error("Erreur log terminal:", error);
        }
    };
                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                            <div className="bg-[#1a1c23] p-4 text-white flex justify-between items-center">
                                <h2 className="text-lg font-bold">Nouveau Admin / Docteur</h2>
                                ...
                                filteredUsers.map((u) => (
                                    <tr key={u.id} className="hover:bg-gray-50/80 transition-colors">
                                        <td className="px-6 py-4 flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-[#00a5a8]/10 flex items-center justify-center text-[#00a5a8] font-bold text-sm">
                                                {u.prenom?.[0]}{u.nom?.[0]}
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-gray-800 m-0">{u.prenom} {u.nom}</p>
                                                <p className="text-[10px] text-gray-400 uppercase font-semibold m-0">{u.role} {u.specialite && `| ${u.specialite}`}</p>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${u.status === 'blocked' ? 'bg-red-100 text-red-600' : u.status === 'archived' ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>
                                                {u.status || 'actif'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex justify-center gap-2">
                                                {(u.status === 'blocked' || u.status === 'archived') && (
                                                    <button onClick={() => handleUpdateStatus(u, 'actif')} className="p-2 rounded-lg bg-green-100 text-green-700 hover:bg-green-600 hover:text-white transition-all shadow-sm flex items-center gap-1" title="Rétablir">
                                                        <RotateCcw size={14} /> <span className="text-[9px] font-bold">Rétablir</span>
                                                    </button>
                                                )}
                                                {u.status !== 'archived' && <button onClick={() => handleUpdateStatus(u, 'archived')} className="p-1.5 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-500 hover:text-white transition-all shadow-sm" title="Archiver"><Archive size={14} /></button>}
                                                {u.status !== 'blocked' && <button onClick={() => handleUpdateStatus(u, 'blocked')} className="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-500 hover:text-white transition-all shadow-sm" title="Bloquer"><Ban size={14} /></button>}
                                                <button onClick={() => handleDelete(u)} className="p-1.5 rounded-lg bg-gray-100 text-gray-500 hover:bg-black hover:text-white transition-all shadow-sm" title="Supprimer"><Trash2 size={14} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
